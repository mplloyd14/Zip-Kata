...
var updateData = function (entry) {
    ...
    return ZipCollection.findZipById(entry._id).then(function (newData) {
        console.log('Data updating');

        newData.city = entry.city;
        newData.pop = entry.pop;
        newData.state = entry.state;
        newData.loc[0] = entry.loc[0];
        newData.loc[1] = entry.loc[1];

        ZipCollection.updateData(newData).then(function (result) {
